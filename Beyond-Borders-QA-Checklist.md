...
- **Environment:** manual read-only checks can target the live site. Integration, Playwright, and `test:all` must use a dedicated disposable Supabase test project — the harness deliberately refuses the production project and requires `ALLOW_E2E_DB_MUTATION=true`.
...
- **Payments:** Preview/local uses Seylan MTF (`test-seylan.mtf…`); Production uses `https://seylan.gateway.mastercard.com` with a separate live MID/password/32-character notification secret. Keep Production payments disabled until MTF coverage and a controlled low-value live canary are complete.
...
- [ ] **RC4 — Pay page: legal acknowledgements before paying.** **Test:** open `/pay/<token>`; the **Pay** button is disabled until both Terms and Privacy are checked. Open each linked document. **Expect:** both open in matching modal dialogs without leaving checkout; both acknowledgements are required and their server-owned document versions are archived with the session. *(§D8 / §O)*
...
- [ ] **D4 — Currency model (USD end to end).** **Test:** compare the package, order, Hosted Checkout, and Merchant Administration values. **Expect:** each uses the same uppercase **USD** amount with two decimal places; there is no client-side or checkout-time conversion. Confirm the production MID is enabled for USD and `MPGS_CURRENCY=USD`.
...
- [ ] **D8 — Payment edge cases & legal gate.** **Test:** (a) leave either Terms or Privacy unchecked; (b) double-click Pay or submit from two tabs; (c) reload the result page after capture; (d) POST to `/api/payments/create-session` from a different origin. **Expect:** (a) payment remains disabled and the API also requires both acknowledgements; (b) one recent MPGS session is reused and optimistic locking prevents competing creation/downgrading a capture; (c) idempotent — no duplicate invoice; (d) cross-origin request is refused with 403. Create-session is rate-limited (**20 / 10 min per IP**, 429 + `Retry-After`).
- [ ] **D9 — Webhook reconciliation.** **Test (technical):** simulate the MPGS webhook `POST /api/payments/webhook` with the `X-Notification-Secret`, `X-Notification-ID`, and `X-Notification-Attempt` headers. **Expect:** wrong/missing secret is rejected using a constant-time comparison; valid JSON with an order ID is acknowledged quickly with 200 and reconciliation continues via `after()`; duplicate delivery is idempotent; the scheduled pending-payment reconciler is the recovery path for deferred failures.
...
- [ ] **K6 — Env / config sanity (go-live).** **Test:** confirm on the live deployment: `NEXT_PUBLIC_SITE_URL=https://www.beyondborders.lk`, `MPGS_BASE_URL=https://seylan.gateway.mastercard.com`, API v100, non-TEST live MID/password, gateway-issued 32-character webhook secret, `MPGS_CURRENCY=USD` (matching Seylan provisioning), and SMTP/SMS configuration. Preview must retain the separate MTF credentials.
